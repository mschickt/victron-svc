...
- `boundary/VictronResource` — JAX-RS endpoints under `/api/victron/*` (`dashboard`, `mppt`, `shunt`, `orion`, `battery`). `ble/devices` delegates to `boundary/BleInventory` for the list of all nearby BLE devices (setup purposes — finding MACs/keys). Prometheus metrics live at `/q/metrics` (Micrometer), health at `/q/health`.
- `boundary/BleInventory` — snapshot of all currently visible BLE devices (not just configured ones), always filtered through `config/BleBlacklist` (`victron.ble.blacklist-names`/`blacklist-macs`, comma-separated; each name entry is a case-insensitive glob pattern matched against the whole name, `*`/`?` wildcards, no wildcard = exact match, e.g. `N0*`) — there is no mode that bypasses the blacklist. Dedupes by normalized MAC (BlueZ can report the same address as multiple objects) and sorts by name. Also runs a periodic inventory scan (`victron.ble.inventory-interval`, default `5m`) that logs the snapshot, persists each device to `ble_scan_reading` via `control/ReadingRepository`, and updates the `victron_ble_inventory_devices` Micrometer gauge.
...
- `control/ReadingRepository` — appends every decoded advertisement/poll to a local SQLite DB (`data/db/victron.db`, override via `VICTRON_DB_PATH`). Tables `mppt_reading`, `shunt_reading`, `orion_reading`, `battery_reading` are created on startup; one row per BLE frame/poll cycle. `ble_scan_reading` stores one row per device per periodic BLE inventory scan (`boundary/BleInventory`), with `manufacturer_ids` as a comma-joined string.
